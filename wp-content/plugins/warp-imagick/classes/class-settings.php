...
if ( ! class_exists( __NAMESPACE__ . '\Settings' ) ) {
	...
	# region Settings Extended
		...
		private function auto_update_disabled_warning() {
			...
			if ( \defined( 'EASY_UPDATES_MANAGER_VERSION' ) ) {
				return;
			}
			...
			\add_action(
				'admin_notices', function() {
					$this->plugin->admin_notice(
						__( 'Warp iMagick plugin \'auto-update\' is disabled or automatic plugin updates are not managed by WordPress automatic updater.' ) .
						'<a style="float:right" target=_blank href="/wp-admin/plugins.php">' . __( 'Open Plugins page.' ) . '</a>' .
	...
	# region Plugin Manager
		...
		protected function on_activate_plugin_success( $networkwide, $user = false ) {
			...
			parent::on_activate_plugin_success( $networkwide, $user );
		}
		...
		protected function on_deactivate_plugin( $networkwide ) {
			...
			parent::on_deactivate_plugin( $networkwide );
		}
			...
			if ( $instance ) {
				...
				if ( \is_multisite() ) {
					...
					foreach ( $sites as $site ) {
						...
						\delete_option( $instance->get_plugin()->get_option_id() . '_disabled' );
